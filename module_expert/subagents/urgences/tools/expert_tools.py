...
async def search_emergency_cases(symptoms: str, tool_context: ToolContext) -> str:
    """
    Recherche des cas cliniques similaires dans la base de connaissances
    pour le domaine : Urgences.
    ...
    try:
        ...
        if not results:
            return "Aucun cas similaire trouvé en Urgences."
            
        return json.dumps(results, ensure_ascii=False, indent=2)

    except Exception as e:
        return f"Erreur lors de la recherche en Urgences: {str(e)}"

search_emergency_cases = FunctionTool(func=search_emergency_cases)
