...
async def search_pneumo_cases(symptoms: str, tool_context: ToolContext) -> str:
    """
    Recherche des cas cliniques similaires dans la base de connaissances
    pour le domaine : Pneumologie.
    ...
    try:
        ...
        if not results:
            return "Aucun cas similaire trouvé en Pneumologie."
            
        return json.dumps(results, ensure_ascii=False, indent=2)

    except Exception as e:
        return f"Erreur lors de la recherche en Pneumologie: {str(e)}"

search_pneumo_cases = FunctionTool(func=search_pneumo_cases)
