...
async def search_endo_cases(symptoms: str, tool_context: ToolContext) -> str:
    """
    Recherche des cas cliniques similaires dans la base de connaissances
    pour le domaine : Endocrinologie.
    ...
    try:
        ...
        if not results:
            return "Aucun cas similaire trouvé en Endocrinologie."
            
        return json.dumps(results, ensure_ascii=False, indent=2)

    except Exception as e:
        return f"Erreur lors de la recherche en Endocrinologie: {str(e)}"

search_endo_cases = FunctionTool(func=search_endo_cases)
