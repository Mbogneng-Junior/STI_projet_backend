...
async def search_internal_cases(symptoms: str, tool_context: ToolContext) -> str:
    """
    Recherche des cas cliniques similaires dans la base de connaissances
    pour le domaine : Médecine interne.
    ...
    try:
        ...
        if not results:
            return "Aucun cas similaire trouvé en Médecine interne."
            
        return json.dumps(results, ensure_ascii=False, indent=2)

    except Exception as e:
        return f"Erreur lors de la recherche en Médecine interne: {str(e)}"

search_internal_cases = FunctionTool(func=search_internal_cases)
