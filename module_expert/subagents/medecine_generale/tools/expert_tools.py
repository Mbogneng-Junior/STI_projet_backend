...
async def search_general_cases(symptoms: str, tool_context: ToolContext) -> str:
    """
    Recherche des cas cliniques similaires dans la base de connaissances
    pour le domaine : Médecine générale.
    ...
    try:
        ...
        if not results:
            return "Aucun cas similaire trouvé en Médecine générale."
            
        return json.dumps(results, ensure_ascii=False, indent=2)

    except Exception as e:
        return f"Erreur lors de la recherche en Médecine générale: {str(e)}"

search_general_cases = FunctionTool(func=search_general_cases)
